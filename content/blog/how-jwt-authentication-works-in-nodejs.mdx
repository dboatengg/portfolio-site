...
That's fine with a handful of users. But think about a site like Instagram or Twitter. Millions of people are logged in at once, all sending requests every second: scrolling, posting, liking, etc. 
One server, no matter how powerful, can't handle that much traffic alone. So companies run many servers instead, and spread incoming requests across them.
...
Now, notice what just disappeared: there's no shared record store anymore. No **Server A** creating something **Server B** has never seen. No syncing, no real-time updates between servers. Each server can verify your token completely on its own, the moment it receives it.
